...
# class GetPath:
#     def __init__(self, db, init_state = "A1A2"):
#         self.cx = []
#         self.cy = []
#         self.cyaw = []
#         self.cv = []
...
#         self.file_open_with_id(init_state)

#     def file_open_with_id(self, id):
#         _,_,self.cx, self.cy, self.cyaw, self.cv = self.db.query_from_id(id)
...
class MPC(Node):
    ...
    def mpc_prob_solve(self, ref_traj, path_predict, x0):
        ...
        if (
            ...
            steer, self.target_idx, _, _ = self.st.stanley_control(self.odom_msg, self.waypoints[: 0], self.waypoints[: 1], self.waypoints[: 2], h_gain = 0.5, c_gain=0.24)
            target_speed = self.waypoints[: 3]
            speed = self.PIDControl(target_speed)
            self.cmd_msg.speed = int(speed) * 10
