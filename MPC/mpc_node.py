...
class mpc_config:
    ...
    Qk: list = field(
        default_factory=lambda: np.diag([8.0, 8.0, 100.0, 100.0])  # levine sim
        # default_factory=lambda: np.diag([50., 50., 5.5, 13.0])
    )  # state error cost matrix, for the the next (T) prediction time steps [x, y, delta, v, yaw, yaw-rate, beta]
    Qfk: list = field(
        default_factory=lambda: np.diag([12.0,12.0, 100.0, 100.0])  # levine sim
        # default_factory=lambda: np.diag([50., 50., 5.5, 13.0])
    ...
    dlk: float = 0.48  # dist step [m] kinematic
