...
from erp42_msgs.msg import AckermannDriveStamped, ControlMessage
...
class mpc_config:
    ...
    Rk: list = field(
        ...
        default_factory=lambda: np.diag([0.005, 400.0])
    )  # input cost matrix, penalty for inputs - [accel, steering_speed]
    Rdk: list = field(
        # default_factory=lambda: np.diag([0.01, 100.0])
        default_factory=lambda: np.diag([0.005, 500.0])
    )  # input difference cost matrix, penalty for change of inputs - [accel, steering_speed]
    Qk: list = field(
        default_factory=lambda: np.diag([8.0, 8.0, 50.0, 13.0])  # levine sim
        # default_factory=lambda: np.diag([50., 50., 5.5, 13.0])
    )  # state error cost matrix, for the the next (T) prediction time steps [x, y, delta, v, yaw, yaw-rate, beta]
    Qfk: list = field(
        default_factory=lambda: np.diag([12.0,12.0, 50.0, 13.0])  # levine sim
        # default_factory=lambda: np.diag([50., 50., 5.5, 13.0])
    ...
    dlk: float = 0.8  # dist step [m] kinematic
...
class MPC(Node):
    ...
    def pose_callback(self, pose_msg):
        ...
        if self.is_real:
            self.vehicle_state.v = -1 * self.vehicle_state.v  # negate the monitoring speed    
        ...
        self.ref_path, self.target_idx = self.calc_ref_trajectory(self.vehicle_state, self.waypoints[:, 0], self.waypoints[:, 1], self.waypoints[:, 2], self.waypoints[:, 3])

        # self.get_logger().info(f"len : {len(self.ref_path[0, :])}")

        # self.get_logger().info(f"target_idx : {self.target_idx}")

        if abs(self.ref_path[0, 7] - self.waypoints[-1, 0]) < 0.3:
            self.get_logger().info("목표지점에 도달했습니다. 시스템 종료 중...")
            rclpy.shutdown()
            return
        # self.get_logger().info(f"ref : {self.ref_path[0, 7]}")
        # self.get_logger().info(f"way : {self.waypoints[-1, 0]}")
        
        self.visualize_ref_traj_in_rviz(self.ref_path)
        
        x0 = [self.vehicle_state.x, self.vehicle_state.y, self.vehicle_state.v, self.vehicle_state.yaw]
        ...
        if self.oa is None or self.odelta_v is None or len(self.oa) == 0 or len(self.odelta_v) == 0:
            ...
            steer, self.target_idx, _, _ = self.st.stanley_control(self.vehicle_state, self.waypoints[:, 0], self.waypoints[:, 1], self.waypoints[:, 2], h_gain=0.5, c_gain=0.24)
            self.cmd_msg.steer = int(m.degrees((-1) * steer))
            target_speed = self.waypoints[0, 3]
            speed = self.pid.PIDControl(self.odom_msg.twist.twist.linear.x, target_speed, 0, 25)
            self.cmd_msg.speed = int(speed) * 36
            self.cmd_msg.gear = 2
